...
public class SupabaseStorageService {
    ...
    /**
     * Rechaza cualquier "path" que intente escapar del bucket (segmentos "..",
     * barras invertidas, o rutas absolutas). El valor recibido aquí puede
     * originarse en un @PathVariable expuesto públicamente (ver
     * ProductoImagenController), así que no debe confiarse en que ya viene
     * saneado por Spring MVC.
     */
    private String sanearPath(String path) {
        if (path == null || path.isBlank()) {
            throw new IllegalArgumentException("La ruta del archivo no puede estar vacía");
        }
        String normalizado = path.replace('\\', '/');
        if (normalizado.startsWith("/") || normalizado.contains("..")) {
            throw new IllegalArgumentException("Ruta de archivo inválida: " + path);
        }
        for (String segmento : normalizado.split("/")) {
            if (segmento.isBlank()) {
                throw new IllegalArgumentException("Ruta de archivo inválida: " + path);
            }
        }
    ...
    private Path resolverLocal(String path) {
        Path destino = localStorageRoot.resolve(path).normalize();
        if (!destino.startsWith(localStorageRoot.normalize())) {
            throw new IllegalArgumentException("Ruta de archivo fuera del almacenamiento local: " + path);
        }
        return destino;
    }
}
