...
public class AuthService {
    ...
    public User registrar(RegisterRequest request) {
        ...
        var cliente = clienteService.resolverOCrearClienteAutenticado(savedUser.getEmail(), savedUser.getName());
        if (request.getDocumentoTipoCodigo() != null && !request.getDocumentoTipoCodigo().isBlank()
                && request.getNumeroDocumento() != null && !request.getNumeroDocumento().isBlank()) {
            cliente.setDocumentoTipoCodigo(request.getDocumentoTipoCodigo());
