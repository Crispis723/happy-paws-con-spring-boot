...
public class AuthController {
    ...
    public AuthController(AuthService authService, PasswordResetService passwordResetService, DocumentoTipoService documentoTipoService) {
    ...
    public String loginForm(@RequestParam(required = false) String error,
                            ...
                            @RequestParam(required = false) String tabExpired,
        ...
        if (locked != null) {
            model.addAttribute("error",
                "Esta cuenta se bloqueó temporalmente por varios intentos fallidos. Intenta de nuevo en unos minutos.");
        }
        if (logout != null) {
            model.addAttribute("success", "Sesión cerrada correctamente");
        }
        if (tabExpired != null) {
            model.addAttribute("info", "Debes iniciar sesión en esta pestaña para continuar.");
        }
    ...
    public String registerUser(@Valid @ModelAttribute("registerRequest") RegisterRequest registerRequest, org.springframework.validation.BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        boolean tieneTipo = registerRequest.getDocumentoTipoCodigo() != null
            && !registerRequest.getDocumentoTipoCodigo().isBlank();
        boolean tieneNumero = registerRequest.getNumeroDocumento() != null
            && !registerRequest.getNumeroDocumento().isBlank();
        if (tieneTipo != tieneNumero) {
            bindingResult.reject("documento.incompleto", "Tipo y número de documento deben registrarse juntos");
        }
