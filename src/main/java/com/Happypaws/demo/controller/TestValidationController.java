 ...
 *
 * SOLO se registra cuando el perfil "dev" está activo
 * (spring.profiles.active=dev). Sin ese perfil, este bean no existe y las
 * rutas /api/test/** devuelven 404 en cualquier otro entorno (producción,
 * Render, etc.), evitando que un usuario autenticado con cualquier rol
 * pueda usar /api/test/email/send como relé de correo arbitrario.
 *
 * Como segunda capa de defensa, SecurityConfig también restringe
 * "/api/test/**" a ROLE_ADMIN por si el perfil "dev" llegara a activarse
 * accidentalmente en un entorno compartido.
 *
 * Rutas:
