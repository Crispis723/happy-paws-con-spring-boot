...
SENDLIB_API_KEY=<API key de Sendlib>
SENDLIB_FROM=Happy Paws <correo Gmail conectado a Sendlib>
...
**Importante:** `JPA_DDL_AUTO` está en `validate` por defecto (ver `application.properties`). Esto significa que Hibernate **no** crea ni modifica tablas automáticamente; solo valida que el esquema de la base de datos coincida con las entidades JPA. Antes de desplegar (o de correr el proyecto localmente por primera vez), ejecuta manualmente el script `docs/supabase-schema-corregido.sql` en tu Supabase — es la única fuente de verdad del esquema, verificada columna por columna contra las entidades del proyecto. Si tu base de datos ya existía antes de que se agregara paginación a Ventas/Compras/Productos/Clientes, corre también `docs/supabase-migration-indices-paginacion.sql` (índices para las columnas de ordenamiento; aditivo, no reconstruye nada). Si tu base de datos ya existía antes de la ficha clínica veterinaria (datos médicos de la mascota, signos vitales del historial, vacunas y desparasitación), corre también `docs/supabase-migration-ficha-clinica-veterinaria.sql` (aditivo). Si tu base de datos ya existía antes de que las citas tuvieran estado (agendada, confirmada, en espera, en consulta, atendida, cancelada, no asistió), corre también `docs/supabase-migration-estados-citas.sql` (aditivo; todas las citas existentes quedan como AGENDADA por defecto). Si tu base de datos ya existía antes de la confirmación de citas por correo (botones "Confirmar"/"Cancelar" en el recordatorio), corre también `docs/supabase-migration-confirmacion-citas.sql` (aditivo; requiere haber corrido antes la migración de estados de citas). Si tu base de datos ya existía antes del módulo de hospitalización, corre también `docs/supabase-migration-hospitalizacion.sql` (aditivo; crea las tablas y los permisos nuevos — DataInitializer asigna esos permisos a los roles automáticamente en el siguiente arranque). Si tu base de datos corrió el script de esquema antes de que se corrigiera un error en la ruta de los módulos de configuración (comprobantes/documentos/unidades/afectaciones apuntaban a la ruta genérica `/configuracion` en vez de la suya propia), corre `docs/supabase-migration-fix-modulos-configuracion.sql` (correctiva, segura de correr varias veces). Los demás scripts SQL en `docs/` (`supabase-migration-add-subtotal-igv-detalles.sql`, `supabase-migration-owner-chip-services.sql`) son migraciones incrementales para bases de datos creadas antes de esos cambios; si tu base de datos es nueva, `supabase-schema-corregido.sql` ya los incluye y no hace falta correrlos. Los scripts en `docs/archive/` son versiones anteriores que ya no coinciden con las entidades actuales; se conservan solo como referencia histórica.
...
- La app expone `GET /actuator/health` (público, sin detalles internos) para que Render verifique que la instancia está viva. `render.yaml` ya lo usa como `healthCheckPath`.
- `.github/workflows/ci.yml` corre `./mvnw clean verify` en cada push/PR a `main`.
...
```powershell
$env:DB_URL="jdbc:postgresql://aws-1-us-east-1.pooler.supabase.com:5432/postgres?sslmode=require"
